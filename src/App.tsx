...
import {
  ...
  fetchDispatchedTasks, confirmTask, fetchKintoneUsers, updateRowByRecordId,
} from './api/workDayApi';
...
const App = () => {
  ...
  const handleSaveDispatchTask = async (task: DispatchedTask, updatedRow: WorkRow) => {
    try {
      await updateRowByRecordId(task.recordId, updatedRow);
      setDispatchedTasks(prev => prev.map(t => t.subtableId === updatedRow.subtableId ? { ...updatedRow, recordId: task.recordId } : t));
    } catch (e) {
      console.error(e);
    }
  };
  ...
  const handleDragStart = (event: any) => {
    ...
    if (data.current?.type === 'source') {
      ...
      setActiveSource({ id: row.subtableId, 標籤: row.來源標籤, 標籤類別: '交辦', 更新時間: '', 最後取用時間: '', WAR_MEMO: '' });
  ...
  return (
    <DndContext sensors={sensors} onDragStart={handleDragStart} onDragEnd={handleDragEnd}>
      <div className="app-layout">
        ...
        <div className="right-panel">
          <div className="week-nav">
            ...
            <div className="week-nav__filters">
              ...
              <button className={`week-nav__filter-btn partial ${taskFilter === 'partial' ? 'active' : ''}`} onClick={() => toggleTaskFilter('partial')} title="部分完成">▲</button>
          ...
          {visibleDates.map(date => (
            <DayColumn
              ...
              dayKey={date}
              date={date}
              rows={applyTaskFilter(workDays?.[date]?.rows || [])}
              recordId={workDays?.[date]?.id || null}
              recordNumber={workDays?.[date]?.記錄號碼 || ''}
        ...
        <DispatchPanel tasks={dispatchedTasks} allTags={allTags} onConfirm={handleConfirmDispatch} onSave={handleSaveDispatchTask} />
